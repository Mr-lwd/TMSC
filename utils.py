...
def encode_text_with_prompt_ensemble(model, obj, device, cnn, adapter):
    # User request: remove trailing digits from category name (e.g. pcb1->pcb, macaroni2->macaroni)
    obj = obj.rstrip('0123456789')
    ...
    prompt_state = [prompt_normal, prompt_abnormal, prompt_background]
    prompt_templates = [
        "a bad photo of a {}.",
    ...
    for i in range(len(prompt_state)):
        prompted_sentence = []
        # prompt_state items no longer use .format(obj) as they are category-agnostic
        # We can just use them directly as 's' to fill the template
        prompted_state = prompt_state[i] 
        ...
        class_embeddings = model.encode_text(
            text=prompted_sentence, cnn=cnn, device=device, adapter=adapter
        )
...
def _read_first_existing_text_file(paths):
    for p in paths:
        try:
            with open(p, "rb") as f:
                raw = f.read()
...
def jetson_model_to_suffix(model):
    if not model:
        return "unknown"
    m = model.strip().lower()
    m = re.sub(r"\s+", " ", m)
    ...
    if "orin" in m and "nx" in m:
        return "orin_nx"
    if "agx orin" in m or ("orin" in m and "agx" in m):
        return "agx_orin"
    if "xavier" in m and "nx" in m:
        return "xavier_nx"
    if "agx xavier" in m or ("xavier" in m and "agx" in m):
        return "agx_xavier"

    s = re.sub(r"[^a-z0-9]+", "_", m).strip("_")
    s = re.sub(r"_+", "_", s)
    return s or "unknown"


def get_jetson_suffix():
    return jetson_model_to_suffix(get_jetson_model())


def get_device_unique_id():
    raw = _read_first_existing_text_file(
        [
...
def get_device_unique_id_short(length=None):
    s = get_device_unique_id()
    if not s:
        return "unknown"
    if length is None:
        return s
    return s[: int(length)] if length else s


def with_jetson_suffix(branch, prefix="jetson"):
    if branch is None:
        branch = ""
    branch = str(branch).strip()
    if branch.startswith(prefix + "_"):
        return branch
    if branch == "" or branch == prefix:
        return f"{prefix}_{get_jetson_suffix()}"
    return branch


def with_jetson_identity(branch, prefix="jetson", id_length=None):
    branch = with_jetson_suffix(branch, prefix=prefix)
    if branch == prefix:
        return branch
    uid = get_device_unique_id_short(id_length)
    if str(branch).endswith("_" + uid):
        return branch
    return f"{branch}_{uid}"
