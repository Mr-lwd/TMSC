    ...
    def get_image_data(self):
        ...
        for classname in self.classnames_to_use:
            ...
            for anomaly in anomaly_types:
                ...
                # Filter out non-image files if necessary, or rely on extension check later
                # For now, just ensure we don't pick up directories inside anomaly folder
                imgpaths_per_class[classname][anomaly] = [
                    os.path.join(anomaly_path, x) for x in anomaly_files if not x.startswith('.')
                ]
                ...
                if self.split == DatasetSplit.TEST and anomaly != "ok":
                    ...
                    maskpaths_per_class[classname][anomaly] = [
                        os.path.join(anomaly_mask_path, x) for x in anomaly_mask_files if not x.startswith('.')
                    ]
                else:
