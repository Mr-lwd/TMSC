...
def _load_state_dict_from_path_or_url(path_or_url: str, *, check_hash: bool = False) -> dict:
    ...
    if parsed.scheme == "file":
        ...
        if local_path.exists():
            try:
                ...
                return torch.load(local_path, map_location="cpu")
    url = convert_path_or_url_to_url(path_or_url)
    return torch.hub.load_state_dict_from_url(url, map_location="cpu", check_hash=check_hash)
    ...
    **kwargs,
    ...
    if pretrained:
        if type(weights) is Weights and weights not in {Weights.LVD1689M, Weights.SAT493M}:
            ...
            state_dict = _load_state_dict_from_path_or_url(weights, check_hash=check_hash)
    ...
    **kwargs,
    ...
    if pretrained:
        if type(weights) is Weights and weights not in {Weights.LVD1689M, Weights.SAT493M}:
            ...
            state_dict = _load_state_dict_from_path_or_url(weights)
        model.load_state_dict(state_dict, strict=True)
    return model
